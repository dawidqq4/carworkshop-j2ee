...
public class ChargeMoneyBean implements Serializable {
    ...
    @PostConstruct
    public void init() {
    	customerOrders = customerOrderManager.findAllWorkerCustomerOrders("finished", userLoginView.getWorker().getId());
    ...
    public void chargeMoney(ActionEvent event) throws IOException {
		...
		String body = "Witamy \n";
		body += "Adres e-mail zosta³ wys³any automatycznie w zwi¹zku z zakoñczeniem zamówienia.\n";
		body += "Pozdrawiamy\n" + "Zespó³ warsztatu samochodowego";
		mailSendingService.sendEmail(selectedCustomerOrder.getCar().getCustomer().getEmailAddress(), 
				"Warsztat samochodowy - dziêkujemy za skorzystanie z naszych us³ug.", body);
